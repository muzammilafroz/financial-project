...
The analysis is grounded in four strands of literature. The first studies growth as a sequence of regimes rather than a single average. \citet{hausmann_pritchett_rodrik_2004} define growth accelerations as sustained departures from prior trends, while \citet{bai_perron_1998,bai_perron_2003} provide the econometric framework for estimating multiple unknown breaks in linear models. \citet{zeileis_leisch_hornik_kleiber_2002} document the \textit{strucchange} implementation used in applied work. Once breaks are identified, two post-break growth specifications matter. The kinked exponential model of \citet{boyce_1986} imposes continuity and interprets breaks as changes in growth rates. A segment-by-segment log-linear trend, by contrast, allows intercept and slope to vary by regime and therefore permits discontinuous level shifts. This distinction is important because \citet{balakrishnan_parameswaran_2007} discuss Indian growth under the broader structural-change framework in which pure and partial breaks need to be separated carefully.
...
The third strand studies regional growth and convergence among Indian states. \citet{sachs_bajpai_ramiah_2002} show that regional growth depends on structural conditions, while \citet{bhattacharya_sakthivel_2004}, \citet{bandyopadhyay_2006}, and \citet{sodsriwiboon_kalra_2010} document divergence, polarisation, club formation, and weak spillovers among Indian states. The recent public-finance evidence in \citet{govil_chauhan_2024} is especially relevant because Arunachal Pradesh's growth cannot be separated from fiscal capacity and central transfers. This literature motivates the comparator set used here: Assam as a Northeast regional baseline, Sikkim as a small Himalayan benchmark, Himachal Pradesh as a hill-state counterfactual outside the Northeast, and Tripura and Meghalaya as additional Northeast comparators.

The fourth strand studies spatial inflation and the COVID-19 shock. \citet{kundu_kishore_bhoi_2018} and \citet{jha_dhal_2019} show that regional inflation dispersion persists even under a common national monetary policy, especially through food and supply-side channels. \citet{mahajan_tomar_2020} and \citet{narayanan_saha_2021} document the food-supply disruption caused by the pandemic, while \citet{beyer_francobedoya_galdo_2020} show that the COVID-19 economic shock varied across Indian states. These studies are directly relevant for Arunachal Pradesh because terrain, distance from major supply centres, and the thinness of local markets can amplify food and fuel price volatility.
...
When breaks are detected, the baseline growth specification is the Boyce kinked exponential model:
\begin{equation}
\ln Y_t = \alpha + \beta_1 t + \sum_{j=1}^{m}\delta_j(t-T_j)D_{jt} + u_t,
\end{equation}
where $T_j$ is the $j$th break date and $D_{jt}=1$ if $t>T_j$. The post-break regime-$j$ growth coefficient is therefore $\beta_1+\delta_1+\cdots+\delta_j$. The specification permits the slope to change at a break while maintaining continuity in the level of log output. This is appropriate for trend-growth estimation because structural breaks in growth need not imply jumps in the level of output \citep{boyce_1986}. Heteroskedasticity-robust standard errors for regime-specific slopes are computed with the delta method from the pooled covariance matrix, and bootstrap confidence intervals are reported for the baseline growth rates.

The paper also reports a discontinuous sensitivity specification:
\begin{equation}
\ln Y_t = \alpha_j + \beta_j t + u_t \qquad \text{for } t \in \text{regime } j.
\end{equation}
Here, each Bai-Perron regime is estimated separately, so both intercepts and slopes may differ across regimes. This segment-wise OLS specification is useful because it produces direct regime-specific standard errors and can reveal fitted level shifts at the break dates. It should not be called the Boyce kinked model, because the point of the Boyce restriction is precisely to rule out such jumps. Following \citet{balakrishnan_parameswaran_2007}, the paper therefore treats the continuous pooled equation as Baseline A and the discontinuous segment-wise regression as Baseline B. Under fixed break dates and an imposed continuity restriction, Baseline A and Baseline B can deliver similar slope estimates while implying different interpretations of what happened at the break itself.
...
Table \ref{tab:growth-baseline-b} shows what changes when continuity is relaxed. For Arunachal Pradesh, the segment-wise OLS sensitivity check yields CAGR estimates of 8.65, 7.19, and 4.56 percent across the three regimes. That is not a reversal of the baseline story: growth is still strongest in the earliest regime and weakest after 2013. The important difference is inferential rather than directional. Once a discontinuous specification is allowed, the 1995-96 break is associated with a fitted level shift of about -17.75 percent, while the 2013-14 break is associated with a smaller positive fitted shift of 3.49 percent. In other words, Baseline B tells a mixed story of slower later growth plus possible jumps in the fitted level of output at regime boundaries. Baseline A is therefore retained as the main specification for trend-growth interpretation, while Baseline B is read as evidence that the 1995 break may bundle a level re-alignment with a slope change.

\begin{table}[H]
\centering
\caption{Baseline A and Baseline B growth-rate comparison}
\label{tab:growth-baseline-b}
\small
\begin{tabular}{L{2.6cm}C{2.0cm}C{1.6cm}C{1.6cm}C{1.8cm}C{1.8cm}}
\toprule
Series & Period & A CAGR & B CAGR & B robust SE & B level shift \\
\midrule
All-India & 1950-1971 & 3.88 & 3.92 & 0.00058 & n.a. \\
All-India & 1972-1978 & 2.51 & 4.74 & 0.00326 & -6.39 \\
All-India & 1979-2003 & 5.53 & 5.56 & 0.00041 & -8.68 \\
All-India & 2004-2018 & 6.92 & 6.80 & 0.00068 & 4.59 \\
All-India & 2019-2025 & 4.93 & 6.57 & 0.01006 & -8.42 \\
Arunachal Pradesh & 1980-1995 & 7.63 & 8.65 & 0.00171 & n.a. \\
Arunachal Pradesh & 1996-2013 & 6.61 & 7.19 & 0.00187 & -17.75 \\
Arunachal Pradesh & 2014-2024 & 5.54 & 4.56 & 0.00280 & 3.49 \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize Notes: CAGR entries are in percent. Baseline A is the continuity-restricted Boyce pooled regression. Baseline B is segment-wise OLS estimated separately within each Bai-Perron regime. ``B level shift'' reports the fitted percentage jump at the start of the regime relative to the previous regime's projected level.
\end{flushleft}
\end{table}
...
Second, the alternative growth specification does not overturn the qualitative result, but it clarifies the econometric stakes. Appendix Figures \ref{fig:india-segmentwise} and \ref{fig:ar-segmentwise} overlay the continuity-restricted and discontinuous fits. For Arunachal Pradesh, the two specifications trace similar broad trajectories, but the segment-wise fit sits above the continuity-restricted line around the 1995-96 break and below it in the final regime. This is consistent with Table \ref{tab:growth-baseline-b}: the main disagreement is not whether growth slowed, but whether the 1995 break should be read purely as a kink in trend growth or partly as a level re-alignment.

Third, the state-comparator module shows that Arunachal Pradesh is a high-growth state from a low base, but not the strongest post-2003 or post-COVID performer. Figure \ref{fig:real-paths} indexes real GSDP paths, while Figure \ref{fig:liberalisation} compares liberalisation-era growth. Sikkim is the most dramatic small-state high-growth benchmark after 2003. Assam is weaker before 2013 but outperforms Arunachal Pradesh after 2013. Tripura's 1991-2003 and 2003-13 performance is stronger than Arunachal Pradesh's. Appendix Figure \ref{fig:sigma} is consistent with the broader divergence literature: cross-state dispersion in log per-capita NSDP does not show a clean monotonic decline, which is exactly why unconditional catch-up is the wrong prior for a frontier state \citep{sachs_bajpai_ramiah_2002,bhattacharya_sakthivel_2004,bandyopadhyay_2006}. These comparisons sharpen the paper's thesis: Arunachal Pradesh's growth is impressive in levels and early growth rates, but it is not a uniquely successful post-reform growth acceleration.
...
This paper provides a complete empirical assessment of Arunachal Pradesh's growth and inflation performance under the assignment brief. The evidence shows a state that grew rapidly but did not experience a standard post-1991 acceleration. Its structural breaks are dated at 1995-96 and 2013-14; its trend growth declines across regimes; and its pre-liberalisation growth exceeds its 1991-2003 growth. A discontinuous segment-wise sensitivity check does not overturn that broad deceleration result, but it does suggest that the 1995-96 break may combine a level re-alignment with a slope change. Inflation is slightly higher on average and clearly more volatile than all India, with a marked fuel component whose premium persists even after the pandemic. The COVID-19 output shock was real but not the most severe among comparable hill and Northeast states, while the recovery has been weaker than several comparators.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig1b_india_segmentwise_vs_kinked_fit.png}
\caption{All-India continuous kinked fit and segment-wise OLS fit}
\label{fig:india-segmentwise}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig4b_ar_segmentwise_vs_kinked_fit.png}
\caption{Arunachal Pradesh continuous kinked fit and segment-wise OLS fit}
\label{fig:ar-segmentwise}
\end{figure}
